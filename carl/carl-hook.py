...
DEEPCLAUDE_MODE_FILE = Path.home() / '.deepclaude-mode'
DEEPCLAUDE_BACKENDS = ('deepseek', 'anthropic')
...
# =============================================================================
# DEEPCLAUDE BACKEND SWITCHING (mid-session, zero model involvement)
# =============================================================================

def read_mode_file() -> str:
    """Return current mode from the global deepclaude mode file."""
    try:
        return DEEPCLAUDE_MODE_FILE.read_text().strip()
    except Exception:
        return 'deepseek'  # safe default


def write_mode_file(mode: str) -> bool:
    """Write a backend mode to the global deepclaude mode file."""
    try:
        DEEPCLAUDE_MODE_FILE.write_text(mode)
        return True
    except Exception:
        return False


def handle_deepclaude_mode(
    user_prompt: str,
    session_config: dict,
    carl_path: Path
) -> tuple:
    """
    Detect and execute backend mode-switches before the model call.
    Returns (mode_message: str, updated_session_config_needs_save: bool).

    Detects patterns from slash-command templates:
      - 'echo deepseek > ...\\.deepclaude-mode'  → switch to DeepSeek
      - 'echo anthropic > ...\\.deepclaude-mode' → switch to Anthropic
      - 'type "...\\.deepclaude-mode"'           → status check

    On switch: writes global mode file + stores preference in session.
    On every request: writes session's stored preference to global file
      (last-writer-wins session isolation — each session claims the global
      mode on its own requests).
    """
    mode_message = ''
    needs_save = False
    prompt_lower = user_prompt.lower() if user_prompt else ''

    # Detect explicit mode-switch intent from slash-command templates
    detected_target = None
    is_status_check = False

    if '.deepclaude-mode' in user_prompt:
        if 'echo deepseek >' in prompt_lower:
            detected_target = 'deepseek'
        elif 'echo anthropic >' in prompt_lower:
            detected_target = 'anthropic'
        elif 'type "' in user_prompt and '.deepclaude-mode' in user_prompt:
            is_status_check = True

    if detected_target:
        current = read_mode_file()
        if write_mode_file(detected_target):
            # Store preference in session overrides for per-session isolation
            if session_config:
                overrides = session_config.get('overrides', {})
                overrides['DEEPCLAUDE_BACKEND'] = detected_target
                session_config['overrides'] = overrides
                needs_save = True
            mode_message = (
                '<deepclaude-mode>\n'
                'Backend switched to {} (was {}).\n'
                'Mode file written before this request — effective immediately.\n'
                'The echo command in your prompt has already been executed by the hook.\n'
                'Do NOT attempt to run any echo/type commands targeting .deepclaude-mode.\n'
                '</deepclaude-mode>'.format(detected_target, current)
            )
        else:
            mode_message = (
                '<deepclaude-mode>\n'
                'ERROR: Failed to write mode file. Backend remains at {}.\n'
                '</deepclaude-mode>'.format(current)
            )
        ...
        mode_message = (
            '<deepclaude-mode>\n'
            'Current backend: {}\n'
            'Router: http://127.0.0.1:3199 → .deepclaude-mode → {} proxy\n'
            'Auto-fallback: Anthropic 401/402/403 → DeepSeek (transparent)\n'
            '</deepclaude-mode>'.format(current, current)
        )
    else:
        # No switch intended — write session preference to global file
        # (session claims the global mode for its own requests)
        if session_config:
            pref = (
                session_config.get('overrides', {}).get('DEEPCLAUDE_BACKEND')
            )
            if pref and pref in DEEPCLAUDE_BACKENDS:
                current = read_mode_file()
                if pref != current:
                    write_mode_file(pref)

    return mode_message, needs_save
...
def main():
    if os.environ.get("CLAUDE_HOOKS_DISABLED", "").strip() not in ("", "0", "false"):
        sys.exit(0)  # Fas 6 assistant-bench arm B: all hooks off, incl. CARL injection
    ...
    # DeepClaude backend switching (before model call — router reads mode file)
    deepclaude_msg = ''
    if user_prompt:
        deepclaude_msg, dc_needs_save = handle_deepclaude_mode(
            user_prompt, session_config, carl_path
        )
        if dc_needs_save and session_config:
            save_session_config(carl_path, session_config)
    ...
    # Force-load HERMES domain on Hermes/Linux agent
    import platform as _platform
    if _platform.system() != 'Windows' and carl_data and 'HERMES' in domains:
        _hcfg = domains['HERMES']
        if _hcfg.get('state', True):
            _hrules = get_domain_rules_from_json(carl_data, 'HERMES')
            if _hrules:
                always_on_rules['HERMES'] = _hrules
    ...
    # Context deduplication — hash actual rule content so edits to carl.json
    # invalidate stale dedup state even when domain membership is unchanged.
    content_hash = hashlib.sha256(
        json.dumps(
            {"always_on": always_on_rules, "matched": matched_rules, "commands": command_rules},
    ...
    if ENABLE_CONTEXT_DEDUP and session_config:
        ...
        if not signature_changed and not first_prompt:
    ...
    if should_emit_full:
        ...
        # Inject DeepClaude mode switch result (if any)
        if deepclaude_msg:
            context += "\n" + deepclaude_msg + "\n"

        # Inject global STATUS.md on fresh sessions
        if bracket == "FRESH":
            _status = Path.home() / "STATUS.md"
            if _status.exists():
                _sc = _status.read_text(encoding="utf-8").strip()
                if _sc:
                    context += f"\n\n<global-status>\n{_sc}\n</global-status>\n"
