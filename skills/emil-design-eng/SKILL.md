...
description: "Animation and interaction-polish specialist encoding Emil Kowalski's design-engineering craft. Fire when: writing or reviewing CSS animations/transitions; adding motion to interactive components (buttons, modals, drawers, popovers, tooltips, toasts); the user says something 'feels off,' 'feels slow,' or 'doesn't feel right'; choosing easing curves or spring physics; implementing drag/swipe/gesture UX; optimizing animation performance (GPU vs JS, Framer Motion vs CSS, WAAPI); doing a UI interaction review (outputs Before/After/Why table per checklist). Trigger phrases: 'animate', 'transition', 'easing', 'spring', 'gesture', 'drag', 'swipe', 'feels slow', 'feels off', 'animation review', 'motion', 'micro-interaction', 'clip-path animation', 'hardware acceleration', 'prefers-reduced-motion', 'transform-origin'. Skip for: static layout, typography, color, or component structure with no animation scope."
user-invocable: true
---
